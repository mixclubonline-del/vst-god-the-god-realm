...
const REALM_COLORS: Record<string, { primary: string; secondary: string; glyph: string }> = {
  'Multi-Realm':    { primary: '#FFD700', secondary: '#DAA520', glyph: '⚒' },
...
export const RealmPortalTransition: React.FC<RealmPortalTransitionProps> = ({
  ...
  const drawVortex = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, progress: number) => {
    ...
    // Phase 1 (0→0.45): Collapse inward — rings shrink
    // Phase 2 (0.45→0.55): Divine flash — white-gold burst
    // Phase 3 (0.55→1): Expand outward — new realm reveals
    const phase = progress < 0.45 ? 'collapse' : progress < 0.55 ? 'flash' : 'expand';
    let phaseProgress: number;
    if (phase === 'collapse') {
      phaseProgress = progress / 0.45;
    } else if (phase === 'flash') {
      phaseProgress = (progress - 0.45) / 0.1;
    } else {
      phaseProgress = (progress - 0.55) / 0.45;
    }
    ...
    const eased = phase === 'collapse'
      ...
      : phase === 'flash'
        ? Math.sin(phaseProgress * Math.PI) // bell curve for flash
    ...
    if (phase === 'collapse') {
      ...
      ctx.globalAlpha = 0.85 - eased * 0.3;
    } else {
    ...
    // ── Divine Light Flash (midpoint burst) ──
    if (phase === 'flash') {
      const flashR = maxR * 0.6 * eased;
      const flashGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, flashR || 1);
      flashGrad.addColorStop(0, `rgba(255,255,255,${0.9 * eased})`);
      flashGrad.addColorStop(0.2, `rgba(255,240,200,${0.6 * eased})`);
      flashGrad.addColorStop(0.5, `rgba(255,215,0,${0.3 * eased})`);
    ...
    for (let i = 0; i < ringCount; i++) {
      ...
      if (phase === 'collapse') {
        ...
        ringRadius = maxR * 0.05 * (1 + ringProgress);
      } else {
      ...
      ctx.strokeStyle = phase === 'flash' ? '#fff' : colors.primary;
      ctx.lineWidth = 2 + (1 - ringProgress) * 3;
      const ringAlpha = phase === 'collapse' ? eased : phase === 'flash' ? eased * 0.8 : (1 - eased);
      ctx.globalAlpha = (1 - ringProgress) * 0.4 * ringAlpha;
      ctx.stroke();
    }

    // ── Central vortex core ──
    let coreSizeRaw: number;
    if (phase === 'collapse') {
      coreSizeRaw = 5 + eased * 60;
    } else if (phase === 'flash') {
      coreSizeRaw = 65 + eased * 40;
    } else {
      coreSizeRaw = 105 - eased * 100;
    }
    ...
    if (phase === 'flash') {
      ...
      coreGlow.addColorStop(0.3, colors.primary);
    ...
    ctx.globalAlpha = phase === 'collapse' ? eased : phase === 'flash' ? 1 : (1 - eased);
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (phase === 'collapse') {
        ...
        pDist = 20 + Math.sin(pAngle * 5 + progress * 30) * 15;
      } else {
      ...
      const pAlpha = phase === 'collapse' ? eased : phase === 'flash' ? eased * 0.5 : (1 - eased);
      ctx.globalAlpha = 0.6 * pAlpha;
      ctx.fillStyle = phase === 'flash' ? '#fff' : (i % 2 === 0 ? colors.primary : colors.secondary);
      ctx.beginPath();
