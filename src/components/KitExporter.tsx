    ...
    <div className="flex flex-col h-full glass-panel border-l border-white/5 bg-black/20 w-80 shrink-0">
      ...
      <div className="p-6 border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-8 h-8 rounded-lg bg-orange-500/20 border border-orange-500/30 flex items-center justify-center text-orange-400 shadow-[0_0_15px_rgba(255,102,0,0.2)]">
            <Layers size={16} />
          </div>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-1">
          <label className="text-[9px] font-black text-orange-500/60 uppercase tracking-widest ml-1">Kit Name</label>
          <input 
            type="text" 
            value={kitName}
            onChange={(e) => update('kitName', e.target.value)}
            className="w-full bg-black/40 border border-white/10 rounded-md px-3 py-2 text-xs text-white focus:border-orange-500/50 outline-none transition-colors"
            placeholder="Enter Kit Name..."
          />
        </div>
        <div className="space-y-1">
          <label className="text-[9px] font-black text-orange-500/60 uppercase tracking-widest ml-1">Author</label>
          <input 
            type="text" 
            value={kitAuthor}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2">
        <div className="flex items-center justify-between mb-2 px-1">
          ...
          <span className="text-[9px] font-mono text-orange-400">{totalIncluded} items</span>
        </div>
        ...
        {presets.slice(0, 12).map((preset, i) => (
          ...
          >
            ...
            {includedPresets[i] ? (
              <CheckCircle2 size={12} className="text-orange-500 shadow-[0_0_8px_rgba(255,102,0,0.5)]" />
            ) : (
      ...
      <div className="p-6 bg-black/40 border-t border-white/5">
        <div className="flex items-center gap-2 mb-4 p-2 rounded bg-orange-500/5 border border-orange-500/10">
        ...
        <button 
          ...
          onClick={onExport}
        >
          <div className="absolute -inset-0.5 bg-gradient-to-r from-orange-600 to-red-600 rounded-lg blur opacity-40 group-hover:opacity-100 transition duration-300"></div>
