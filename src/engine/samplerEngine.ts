...
interface ModuleNode {
  ...
  wetGain: GainNode;
  dryGain: GainNode;
  internals: Record<string, AudioNode | AudioParam | Array<AudioNode | AudioParam>>;
...
export class GodRealmSamplerEngine {
  ...
  async init(): Promise<void> {
    ...
    // CRITICAL: Gains must start at 0 so the always-running oscillator is silent
    // until triggerMulti808() envelopes the gain on pad hit.
    this.m808MasterGain = this.ctx.createGain();
    this.m808MasterGain.gain.value = 0.8; // Fixed bus level — always audible when sub fires
    this.m808SubGain = this.ctx.createGain();
    this.m808SubGain.gain.value = 0; // Silent until triggerMulti808() envelopes per-hit
    ...
    await this.loadSamples(abortController.signal);
    if (abortController.signal.aborted || this.abortController !== abortController) return;
  ...
  private createModuleNode(mod: DSPChainModule): ModuleNode | null {
    ...
    const internals: Record<string, AudioNode | AudioParam | Array<AudioNode | AudioParam>> = {};
